...
export class DependenciaController {
  ...
  @Get(':dependencia')
  async findOneDependencia(@Param('dependencia') dependencia: string): Promise<CreateDependenciaDto> {
    try {
      return await this.dependenciaService.findOneByDependencia(dependencia);
    } catch (error) {
      // Si el error ya es una HttpException (manejado previamente), lo volvemos a lanzar
  ...
  async updateDependencia(@Param('id', new ParseIntPipe({ errorHttpStatusCode: HttpStatus.NOT_ACCEPTABLE })) id: number, @Body() updateDependenciaDto: UpdateDependenciaDto): Promise<UpdateDependenciaDto> {
    try {
